...
def test_request_generation ():	
	...
	assert request.body.startswith (b"-----") and len (request.body) == 461
	assert request.get_header ('content-length') == "461"
	...
	rprint (request.get_header ('content-length'))
	assert request.get_header ('content-type') == "application/grpc"
