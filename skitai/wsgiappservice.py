...
from skitai.handlers import api_access_handler, vhost_handler
...
class WAS:
    ...
    # async requests -----------------------------------------------
    def assert_params (self, callback, mapreduce = False):
        if is_main_thread () and not callback:
            raise RuntimeError ('Should have callback in Main thread')
        if mapreduce and callback: 
            raise RuntimeError ("Cannot use callback with Map-Reduce")
