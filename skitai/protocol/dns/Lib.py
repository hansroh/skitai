	...
	def addname(self, name):
		...
		for j in range(i):
			...
			if offset + len(buf) < 0x3FFF:
				...
				print('DNS.Lib.Packer.addname: %S warning: pointer too big' % name)
			buf = buf + (chr(n).encode ("utf8") + label)
		if pointer:
...
# Added SRV (RFC 2782) support. Code from Michael Strer.
#
# Revision 1.6  2001/07/19 07:39:18  anthony
# 'type' -> 'rrtype' in getRRheader(). Fix from Michael Strer.
#
